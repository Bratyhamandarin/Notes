      ...
      <div className='container-fluid'>
        <div className='row'>
          <div className='col-xs-12 col-sm-3 col-md-3'>
            <aside>
              <button onClick={this.showMenu} className='btn btn-primary btn-block open'>Открыть меню</button>
              {this.state.menuCollapsed ? null :
                (<div className='menu_opt'>
                  <button onClick={this.handleSubmit} className='btn btn-primary btn-block'>Добавить заметку</button>
                  <button onClick={this.hideMenu} className='btn btn-primary btn-block'>Закрыть меню</button>
                </div>)
              }
            </aside>
          </div>
          <NoteList handleToggle={this.handleToggle}
